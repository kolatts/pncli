    ...
    .action(async (cmdOpts: { output: string }) => {
      ...
      try {
        ...
        if (cmdOpts.output === 'table') {
          ...
          const services = ['jira', 'bitbucket', 'github', 'confluence', 'sonar', 'sde', 'ado', 'jenkins', 'udeploy', 'artifactory', 'checkmarx', 'servicenow', 'contrast', 'sonatypeiq', 'openshift', 'dynatrace', 'dynatrace_platform', 'logscale', 'splitio'] as const;
          const labelWidth = 14;
          const statusWidth = 9;
