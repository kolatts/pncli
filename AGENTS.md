...
### Credential storage is the exception, and it is opt-in

`src/lib/keychain.ts` shells out to the OS's own credential store: `security` on macOS, the built-in Windows PowerShell (advapi32) on Windows, and `secret-tool` on Linux. That does not breach the rule above, because it is **where a secret is kept, never how one is obtained**. Plaintext config and `PNCLI_*` env vars keep working with no keychain at all, and nothing may ever *require* a `keychain:` reference. The same goes for `skills git-auth`, which drives `git` (already a hard dependency of marketplaces) and `git credential`. When extending either:

- Secrets travel to the helper process on **stdin, never argv**. argv is visible to every process on the machine.
- A reference that cannot be resolved becomes `undefined` (the same as an unset key). It never becomes the literal `keychain:...` string sent as a credential.
- Env vars stay above keychain references in precedence (see **Configuration Precedence**).
- Anything that moves a secret (`config keychain migrate` in either direction) must leave it in at least one place at every step. Verify the write, rewrite config, and only then delete.
...
The site does **not** host a skills gallery or catalog — do not re-add one; `skills/pncli/` is the source of truth for skill content and the onboarding contract. What it does have is one **explainer**, `/skills-guide/`, generated by `site/scripts/parse-skills-guide.mjs` from `skills/pncli/skills-guide.md`. That same file is what `pncli skills guide` prints in the terminal and what agents get inside the installed skill, so the in-app and web explanations cannot drift: edit the markdown, never the generated page. Keep angle-bracket placeholders inside code spans (the page is MDX; `src/services/skills/guide.test.ts` enforces it).
...
1. Start the dev server in `site/` with `--host 0.0.0.0` so the Docker-based browser tool can reach it:
   ```bash
   node scripts/parse-changelog.mjs && node scripts/parse-instructions.mjs && node scripts/parse-skills-guide.mjs && node scripts/parse-commands.mjs && node_modules/.bin/astro dev --port 4323 --host 0.0.0.0
   ```
...
- `skills/pncli/skills-guide.md` — screenshot `/skills-guide/`
